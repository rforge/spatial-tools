...
#					image_dims=c(image_dims[2],image_dims[1],image_dims[3])
			...
			x=x,tr=tr)
#	browser()
	writeMapply <- mapply(function(filename,image_dims,r_out,chunk_position,dataType)
			{
#				print(filename)
#				print(image_dims)
#				print(r_out)
...
focal_hpc <- function(x,
		...
		outformat="raster",additional_header="ENVI",dataType="FLT8S",
...
#	browser()
	...
	if(length(dataType)==1) dataType=as.list(rep(dataType,length(filename)))
	
	out <- mapply(function(filename,outbands,dataType,reference_raster,overwrite,verbose)
			{
			...
			},filename=filename,outbands=outbands,dataType=dataType,
