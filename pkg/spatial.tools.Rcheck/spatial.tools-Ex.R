...
##D data1s <- array(1,dim=c(length(data_position[[1]]),length(data_position[[2]]),length(data_position[[3]])))
...
## Not run: 
##D  tahoe_highrez <- brick(system.file("external/tahoe_highrez.tif", package="spatial.tools"))
##D # Pixel-based processing:
##D # Uncomment sfQuickInit()/sfQuickStop() to (potentially) speed this up:
##D # sfQuickInit()
##D 	ndvi_function <- function(x,...)
##D 	{
##D 		# Note that x is received by the function as a 3-d array:
##D 		red_band <- x[,,2]
##D 		nir_band <- x[,,3]
##D 		ndvi <- (nir_band - red_band)/(nir_band + red_band)
##D 		# The output of the function should also be a 3-d array,
##D 		# even if it is a single band:
##D 		ndvi <- array(ndvi,dim=c(dim(x)[1],dim(x)[2],1))
##D 		return(ndvi)
##D 	}
##D  tahoe_ndvi <- focal_hpc(x=tahoe_highrez,fun=ndvi_function)
##D # sfQuickStop()
##D 
...
##D # sfQuickStop()
##D 
##D # Example with 7 x 7 window in parallel mode:
##D sfQuickInit()
