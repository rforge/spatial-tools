#' Model predictions on rasters
#' @param object a model object for which prediction is desired.
#' @param newdata Raster*. Raster* objects to apply the model to.
#' @param ... additional arguments affecting the predictions produced.
#' @author Jonathan A. Greenberg (\email{spatial.tools@@estarcion.net})
#' @seealso \code{\link{predict}}
#' @details predict.rasterEngine will apply a model to a Raster* object,
#' following the basic approach outlined in the generic predict.  Currently,
#' this works for predict.* statements in which the data to predict on is
#' called by the parameter "newdata", the input data is in the form of a 
#' data.frame, and the output is a vector or matrix of numbers or factors.
#' 
#' predict.rasterEngine will run in parallel if a cluster is registered
...
predict.rasterEngine <- function(object,newdata,...)
{
